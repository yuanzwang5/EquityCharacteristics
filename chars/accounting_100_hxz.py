...
data_rawa['rd'] = np.where(((data_rawa['xrd'] / data_rawa['at']) - (data_rawa['xrd/at_l1_l1'])) / data_rawa['xrd/at_l1_l1'] > 0.05, 1, 0)
...
crsp = crsp.dropna(subset=['ret', 'retx', 'prc']).reset_index(drop=True)  # 最后comment
...
crsp_mom['turn'] = ((crsp_mom['vol_l1']+crsp_mom['vol_l2'] + crsp_mom['vol_l3'])/3/1000)/crsp_mom['shrout']
...
# data_rawa
data_rawa = data_rawa.drop(columns=['date', 'ret', 'retx', 'me', 'vol', 'permco', 'prc', 'shrout'], errors='ignore')
data_rawa = pd.merge(crsp_mom, data_rawa, how='left', on=['permno', 'jdate']).reset_index(drop=True)
data_rawa = data_rawa.sort_values(by=['permno', 'jdate']).reset_index(drop=True)
data_rawa['datadate'] = data_rawa.groupby(['permno'])['datadate'].fillna(method='ffill') # avoid the bug of 'groupby' for py 3.8
...
data_rawa = data_rawa.loc[
    ...
    (data_rawa['sharetype'] == 'NS') &
    (data_rawa['securitytype'] == 'EQTY') &
    (data_rawa['securitysubtype'] == 'COM') &
    (data_rawa['usincflg'] == 'Y') &
    (data_rawa['issuertype'].isin(['ACOR', 'CORP']))
].reset_index(drop=True)

# data_rawq
data_rawq = data_rawq.drop(columns=['date', 'ret', 'retx', 'me', 'vol', 'permco', 'prc', 'shrout'], errors='ignore')
data_rawq = pd.merge(crsp_mom, data_rawq, how='left', on=['permno', 'jdate']).reset_index(drop=True)
data_rawq = data_rawq.sort_values(by=['permno', 'jdate']).reset_index(drop=True)
data_rawq['datadate'] = data_rawq.groupby(['permno'])['datadate'].fillna(method='ffill') # avoid the bug of 'groupby' for py 3.8
...
data_rawq = data_rawq.loc[
    ...
    (data_rawq['sharetype'] == 'NS') &
    (data_rawq['securitytype'] == 'EQTY') &
    (data_rawq['securitysubtype'] == 'COM') &
    (data_rawq['usincflg'] == 'Y') &
    (data_rawq['issuertype'].isin(['ACOR', 'CORP']))
].reset_index(drop=True)
